...
V2X
--------------------------
RUUT 를 통해 T Map 에서 서비스 되고 있는 2가지 V2X 서비스인 전방 급정거 알림 및 응급 차량 접근 알림 서비스의 원시 데이터를 능동적으로 획득할 수 있습니다. API 호출을 통하여 서버 URI 및 요청 서비스 데이터를 등록하면 RUUT 에서 매 이벤트 발생 시 마다 등록된 URI 로 V2X 서비스의 원시 데이터를 PUSH 메시지로 전달해 드립니다.
